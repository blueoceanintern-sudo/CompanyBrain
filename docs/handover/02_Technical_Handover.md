...
| 8 | ~~**Chat toolbar placeholders.**~~ **Resolved 2026-07-22.** The "Attach file"/"Web search"/"AI settings" toolbar buttons and the toast-only "Helpful" answer-feedback button were removed in a UX audit rather than left as non-functional UI. | N/A | A real query-feedback feature (replacing "Helpful") is tracked in the Roadmap below. |
...
6. **Query feedback ("was this helpful").** The chat page had a thumbs-up "Helpful" button that only showed a toast — no `helpful`-style column on `queries` (`db/schema/queries.ts`) and no feedback endpoint existed, so it was removed in a 2026-07-22 UX audit rather than left misleading. To build it for real: add a nullable `helpful` boolean (or similar) column via migration, a small `PATCH /orgs/:id/query/:queryId/feedback` route following the route-handler/service split in `CLAUDE.md`, and wire the chat page's thumbs-up button to call it and reflect saved state.
7. `TODO(Tori): anything promised to Equest or in the PRD that isn't listed above — attach or link the PRD here.`
